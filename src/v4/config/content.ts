export const HERO_CONTENT = {
  // Quebras fixas só valem pro desktop. No mobile a `bold` flag de cada
  // segmento se mantém, mas tudo vira uma linha só (sem `<br>`) pra fluir
  // naturalmente — mesmo recurso que `subtitleLinesMobile` já usava.
  // `bold` engrossa o peso; `accent` pinta com a cor da marca — só a linha 1
  // leva destaque (cor, sem engrossar); as outras ficam neutras.
  titleLines: [
    [{ text: 'Tudo o que existe sobre peptídeo', bold: false, accent: true }],
    [{ text: 'reunido num material só,', bold: false, accent: false }],
    [{ text: 'organizado por classe.', bold: false, accent: false }],
  ],
  titleLinesMobile: [
    [
      { text: 'Tudo o que existe sobre peptídeo ', bold: false, accent: true },
  ...
  subtitleLines: [
    [{ text: 'São 6 ebooks que vão do cognitivo ao metabólico,', bold: false }],
    [{ text: 'cada um destrinchando uma classe de peptídeos.', bold: false }],
    [{ text: 'O conteúdo completo pra você parar de catar informação', bold: true }],
    [{ text: 'solta na internet e dominar o assunto de uma vez.', bold: true }],
  ],
  subtitleLinesMobile: [
    [
      {
        text: 'São 6 ebooks que vão do cognitivo ao metabólico, cada um destrinchando uma classe de peptídeos. ',
        bold: false,
      },
      {
        text: 'O conteúdo completo pra você parar de catar informação solta na internet e dominar o assunto de uma vez.',
        bold: true,
      },
    ],
  ],
...
export const PRICE_CONTENT = {
  ...
  ctaHref: 'https://checkout.thebank.com.br/pt-BR/7506719024738136064?cc=BR',
} as const
